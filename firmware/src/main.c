...
int main(void)
{
    ...
    if (rtc_ret == LIMA_RTC_ERR_SKEW) {
        LOG_WRN("[RTC] skew detected — posting tamper event");
        lima_event_t e = {
            .type         = LIMA_EVT_TAMPER_DETECTED,
            .timestamp_ms = (uint32_t)lima_rtc_timestamp_ms(),
        };
        lima_post_event(&e);
    } else if (rtc_ret != LIMA_RTC_OK && rtc_ret != LIMA_RTC_ERR_FALLBACK) {
